...
@property (nonatomic, strong) SVStatusImage *imageView;
@property (nonatomic, strong) UIWindow *previousKeyWindow;

- (void)showWithImage:(UIImage*)image status:(NSString*)string duration:(NSTimeInterval)duration;
- (void)showWithoutImage:(NSString *)string duration:(NSTimeInterval)duration;
...
+ (void)showWithoutImage:(NSString*)string {
    [[SVStatusHUD sharedView] showWithoutImage:string duration:1];
}
...
- (void)showWithoutImage:(NSString *)string duration:(NSTimeInterval)duration {
	...
	self.fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:SVStatusHUDVisibleDuration target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
...
- (UILabel *)stringLabel {
    ...
    if (stringLabel == nil) {
        stringLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 75, 160, 20)];
